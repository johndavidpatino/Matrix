...
- Sprint 3 (Cargas masivas) en progreso: el wizard `/OP/ImportacionMasiva` y sus viewmodels interactúan con `OpCargaService`, que valida headers (`TipoActividad`, cortes/festivos) y luego ejecuta los SP `CatiRMC_*` (borrar, validar, reportes y carga final) y el bulk copy hacia `OP_CuantiPlanillas`, dejando un backup en `uploads/op/cargas`. La vista ahora muestra los reportes (`ResumenValidas`, `ResumenNoValidas`, `ResumenDuplicadas`, `ResumenInconsistencias`) para dar seguimiento inmediato y dar cierre al sprint 3.
