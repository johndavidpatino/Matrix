...
namespace MatrixNext.Infrastructure.Adapters.GD
{
    ...
    public class MaestroDocumentoAdapter : IMaestroDocumentoAdapter
    {
        ...
        public async Task<long> CrearMaestroTipo1ConstruccionAsync(MaestroTipo1ConstruccionDto maestro)
        {
            try
            {
                // SP: GD_MaestroDocumentos_Add (Tipo 1 = Construcción)
                var parameters = new DynamicParameters();
                ...
                parameters.Add("@IdTipoSolicitud", 1); // Tipo 1 = Construcción
                parameters.Add("@Activo", true);
                parameters.Add("@Controlado", maestro.Controlado);
                parameters.Add("@TiempoRetencion", maestro.TiempoRetencionAños);
                ...
                _logger.LogInformation("Maestro Tipo 1 (Construcción) creado. IdMaestro: {IdMaestro}, Nombre: {Nombre}", 
        ...
        public async Task<long> CrearMaestroTipo2ActualizacionAsync(MaestroTipo2ActualizacionDto maestro)
        {
            try
            {
                // SP: GD_MaestroDocumentos_Add (Tipo 2 = Actualización)
                var parameters = new DynamicParameters();
                parameters.Add("@Documento", maestro.NombreDocumento);
                parameters.Add("@Codigo", maestro.CodigoDocumento);
                parameters.Add("@IdProceso", maestro.IdProceso);
                parameters.Add("@IdTipoSolicitud", 2); // Tipo 2 = Actualización
                ...
                _logger.LogInformation("Maestro Tipo 2 (Actualización) creado. IdMaestro: {IdMaestro}, IdMaestroExistente: {IdMaestroExistente}, Versión: {Version}", 
        ...
        public async Task<bool> AnularMaestroTipo3Async(MaestroTipo3AnulacionDto anulacion)
        {
            try
            {
                ...
                await _connection.ExecuteAsync(
                    @"INSERT INTO GD_Auditoría (IdEntidad, TipoEntidad, Accion, Descripción, UsuarioId, FechaAccion)
                      VALUES (@IdMaestro, 'MaestroDocumento', 'Anulación', @Motivo, @UsuarioId, GETDATE())",
                    new { anulacion.IdMaestroAnular, anulacion.MotivoAnulacion, anulacion.UsuarioAnulacion }
                );

                _logger.LogInformation("Maestro Tipo 3 (Anulación) completada. IdMaestro: {IdMaestro}, Motivo: {Motivo}", 
