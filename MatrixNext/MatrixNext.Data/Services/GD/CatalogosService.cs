...
{
    /// <summary>
    /// Interfaz para servicio de CatÃ¡logos
    /// </summary>
    ...
    public class CatalogosService : ICatalogosService
    {
        ...
        public async Task<TipoSolicitudDto> ObtenerTipoSolicitudAsync(long idTipoSolicitud)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("ID de tipo de solicitud invÃ¡lido: {IdTipoSolicitud}", idTipoSolicitud);
                    return null;
                }
            ...
            TipoSolicitudDto tipoSolicitud, long usuarioModifica)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Intento de actualizar tipo de solicitud con ID invÃ¡lido");
                    return (false, "El ID del tipo de solicitud es invÃ¡lido");
                }
            ...
            long idTipoSolicitud, long usuarioModifica)
        {
            try
            {
                if (idTipoSolicitud <= 0)
                {
                    _logger.LogWarning("ID de tipo de solicitud invÃ¡lido para eliminar: {IdTipoSolicitud}",
                        idTipoSolicitud);
                    return (false, "El ID es invÃ¡lido");
                }
        ...
        public async Task<IEnumerable<EstadoDto>> ObtenerEstadosPorModuloAsync(string modulo, bool soloActivos = false)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("MÃ³dulo vacÃ­o para obtener estados");
                    return Enumerable.Empty<EstadoDto>();
                }

                _logger.LogInformation("Obteniendo estados para mÃ³dulo: {Modulo}", modulo);
                return await _adapter.ObtenerEstadosPorModuloAsync(modulo, soloActivos);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error obteniendo estados para mÃ³dulo {Modulo}", modulo);
                return Enumerable.Empty<EstadoDto>();
            }
        }
        ...
        public async Task<EstadoDto> ObtenerEstadoAsync(long idEstado)
        {
            try
            {
                if (idEstado <= 0)
                {
                    _logger.LogWarning("ID de estado invÃ¡lido: {IdEstado}", idEstado);
                    return null;
                }
            ...
            EstadoDto estado, long usuarioModifica)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Intento de actualizar estado con ID invÃ¡lido");
                    return (false, "El ID del estado es invÃ¡lido");
                }
            ...
            long idEstado, long usuarioModifica)
        {
            try
            {
                if (idEstado <= 0)
                {
                    _logger.LogWarning("ID de estado invÃ¡lido para eliminar: {IdEstado}", idEstado);
                    return (false, "El ID es invÃ¡lido");
                }
        ...
        public async Task<ProcesoDto> ObtenerProcesoAsync(long idProceso)
        {
            try
            {
                if (idProceso <= 0)
                {
                    _logger.LogWarning("ID de proceso invÃ¡lido: {IdProceso}", idProceso);
                    return null;
                }
            ...
            ProcesoDto proceso, long usuarioModifica)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Intento de actualizar proceso con ID invÃ¡lido");
                    return (false, "El ID del proceso es invÃ¡lido");
                }
            ...
            long idProceso, long usuarioModifica)
        {
            try
            {
                if (idProceso <= 0)
                {
                    _logger.LogWarning("ID de proceso invÃ¡lido para eliminar: {IdProceso}", idProceso);
                    return (false, "El ID es invÃ¡lido");
                }
        ...
        public async Task<CatalogosResumenDto> ObtenerResumenAsync()
        {
            try
            {
                _logger.LogInformation("Obteniendo resumen de catÃ¡logos");
                return await _adapter.ObtenerResumenAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error obteniendo resumen de catÃ¡logos");
