    ...
    public class PncService : IPncService
    {
        ...
        public async Task<PncDto> ObtenerPncDetalleAsync(long idPnc)
        {
            try
            {
                if (idPnc <= 0)
                {
                    _logger.LogWarning("ID de PNC invÃ¡lido: {IdPnc}", idPnc);
                    return null;
                }
            ...
            PncDto pnc, long usuarioRegistra)
        {
            try
            {
                ...
                if (pnc == null)
                {
                    ...
                    return (false, "El PNC no puede estar vacÃ­o", 0);
                }

                if (!pnc.AsociadoA.HasValue || pnc.AsociadoA <= 0)
                {
                    _logger.LogWarning("PNC sin 'Asociado A' especificado");
                    return (false, "El tipo de asociaciÃ³n es obligatorio", 0);
                }
                ...
                {
                    _logger.LogWarning("PNC sin descripciÃ³n");
                    return (false, "La descripciÃ³n es obligatoria", 0);
                }

                if (usuarioRegistra <= 0)
                {
                    _logger.LogWarning("Usuario de registro invÃ¡lido: {UsuarioId}", usuarioRegistra);
                    return (false, "Usuario invÃ¡lido", 0);
                }
            ...
            {
                _logger.LogError(ex, "Error creando PNC. Usuario: {UsuarioId}, DescripciÃ³n: {Descripcion}",
            ...
            PncDto pnc, long usuarioModifica)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Intento de actualizar PNC con ID invÃ¡lido");
                    return (false, "El ID del PNC es invÃ¡lido");
                }
                ...
                // Validar que pueda editarse (solo si estÃ¡ en estado Registrado)
        ...
        public async Task<IEnumerable<PncCausaDto>> ObtenerCausasAsync(long idPnc)
        {
            try
            {
                if (idPnc <= 0)
                {
                    _logger.LogWarning("ID de PNC invÃ¡lido para obtener causas: {IdPnc}", idPnc);
            ...
            PncCausaDto causa, long usuarioRegistra)
        {
            try
            {
                if (causa == null)
                {
                    ...
                    return (false, "La causa no puede estar vacÃ­a", 0);
                }
                ...
                {
                    _logger.LogWarning("Causa sin descripciÃ³n");
                    return (false, "La descripciÃ³n de la causa es obligatoria", 0);
                }

                if (string.IsNullOrWhiteSpace(causa.AccionCorrectiva))
                {
                    _logger.LogWarning("Causa sin acciÃ³n correctiva");
                    return (false, "La acciÃ³n correctiva es obligatoria", 0);
                }
                ...
                _logger.LogInformation(
                    "Agregando causa al PNC {IdPnc}: DescripciÃ³n={Descripcion}",
        ...
        public async Task<IEnumerable<PncSeguimientoDto>> ObtenerSeguimientoAsync(long idPnc)
        {
            try
            {
                if (idPnc <= 0)
                {
                    _logger.LogWarning("ID de PNC invÃ¡lido para obtener seguimiento: {IdPnc}", idPnc);
        ...
        public async Task<IEnumerable<PncLogDto>> ObtenerLogAsync(long idPnc)
        {
            try
            {
                if (idPnc <= 0)
                {
                    _logger.LogWarning("ID de PNC invÃ¡lido para obtener log: {IdPnc}", idPnc);
