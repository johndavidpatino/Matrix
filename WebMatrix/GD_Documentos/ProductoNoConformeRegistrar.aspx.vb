...
Public Class ProductoNoConformeRegistrar
    ...
    Private Sub GVTrabajos_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GVTrabajos.SelectedIndexChanged
        ...
        JobBook.Text = GVTrabajos.SelectedRow().Cells(2).Text
        NomTrabajo.Text = Server.HtmlDecode(GVTrabajos.SelectedRow().Cells(3).Text)

        Session("IdPNC") = 0

        GVPNCCausas.DataSource = Nothing
        GVPNCCausas.DataBind()
        GVPNCAcciones.DataSource = Nothing
        GVPNCAcciones.DataBind()

        btnGrabarPNC.Visible = True
        btnGrabarCausa.Visible = False
        btnGrabarAccion.Visible = False
        btnActualizarFecha.Visible = False
    End Sub
    Private Sub btnGrabarPNC_Click(sender As Object, e As EventArgs) Handles btnGrabarPNC.Click
        ...
        If WError = False Then
            ...
            If hfId.Value = "0" Then
                ...
                Try
                    oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/NotificacionPNC.aspx?idPNC=" & IdPNC)
    ...
    Private Sub btnGrabarCausa_Click(sender As Object, e As EventArgs) Handles btnGrabarCausa.Click
        Dim IdCausa As Integer
        IdCausa = o.GrabarRegistroPNCCausa(CInt(Session("IdPNC")), txtCausa.Text)
        Session("IdCausa") = IdCausa

        Me.GVPNCCausas.DataSource = o.LstPNCCausas(CInt(Session("IdPNC")))
        GVPNCCausas.DataBind()

        btnGrabarAccion.Visible = True

        ShowNotification("CAUSA GRABADA", WebMatrix.ShowNotifications.InfoNotification)
    End Sub
    Protected Sub btnGrabarAccion_Click(sender As Object, e As EventArgs) Handles btnGrabarAccion.Click
        ...
        If WError = False Then
            'GRABA DETALLE
            IdPNCDetalle = o.GrabarRegistroPNCAccion(CInt(Session("IdPNC")), CInt(Session("IdCausa")), TipoAccion.SelectedValue, txtAccion.Text, FechaPlanAccion.Text, FechaEjecAccion.Text, ResponsableAccion.SelectedValue, ResponsableSeguir.SelectedValue, txtEvidencia.Text)
            Dim oEnviarCorreo As New EnviarCorreo
            Try
                oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/NotificacionPNCAcciones.aspx?idPNCDet=" & IdPNCDetalle & "&JOBBOOK=" & JobBook.Text)
            Catch ex As Exception
            End Try

            LimpiarAccion()

            Me.GVPNCAcciones.DataSource = o.LstPNCAcciones(CInt(Session("IdPNC")), CInt(Session("IdCausa")))
            GVPNCAcciones.DataBind()

            ShowNotification("ACCION GRABADA", WebMatrix.ShowNotifications.InfoNotification)
        End If
    ...
    Protected Sub btnActualizarFecha_Click(sender As Object, e As EventArgs) Handles btnActualizarFecha.Click
        ...
        Else
            o.ActualizarPNCdetalle(Session("IdAccion"), FechaEjecAccion.Text, txtEvidencia.Text)
    ...
    Private Sub ValidarDatosPNC()
        ...
        End If
    End Sub
    Private Sub ValidarDatosAccion()
        ...
        If o.ExisteAccion(CInt(Session("IdPNC")), CInt(Session("IdCausa")), TipoAccion.SelectedValue) = True Then
            ShowNotification("TIPO de ACCION YA EXISTE", WebMatrix.ShowNotifications.ErrorNotification)
            WError = True
        End If

        If (TipoAccion.SelectedValue = 2 Or TipoAccion.SelectedValue = 3) And o.ExisteAccionInmediata(CInt(Session("IdPNC")), CInt(Session("IdCausa"))) = False Then
    ...
    Private Sub GVPNC_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GVPNC.SelectedIndexChanged
        ...
        txtDescripcion.Text = CType(GVPNC.SelectedRow().Cells(7).FindControl("Descripcion"), TextBox).Text
        ...
        If o.ObtenerTarea(CInt(Session("IdPNC"))) > 0 Then
        ...
        Else
            Tarea.DataSource = Nothing
            Tarea.SelectedValue = Nothing
        End If
        ...
        GVPNCCausas.SelectedRowStyle.ForeColor = Drawing.Color.Black
        GVPNCCausas.SelectedRowStyle.Font.Bold = False

        LimpiarAccion()
    End Sub
    Private Sub GVPNCCausas_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GVPNCCausas.SelectedIndexChanged
        If CInt(Session("IdPNC")) > 0 Then
            Session("IdCausa") = GVPNCCausas.SelectedRow().Cells(2).Text

            Me.GVPNCAcciones.DataSource = o.LstPNCAcciones(CInt(Session("IdPNC")), CInt(Session("IdCausa")))
            GVPNCAcciones.DataBind()

            txtCausa.Text = CType(GVPNCCausas.SelectedRow().Cells(3).FindControl("CausaRaiz"), TextBox).Text
            ...
            GVPNCAcciones.SelectedRowStyle.ForeColor = Drawing.Color.Black
            GVPNCAcciones.SelectedRowStyle.Font.Bold = False
        Else
            GVPNCCausas.SelectedRowStyle.ForeColor = Drawing.Color.Black
            GVPNCCausas.SelectedRowStyle.Font.Bold = False
            ShowNotification("DEBE SELECCIONAR UN PNC ANTES DE SELECCIONAR UNA CAUSA", WebMatrix.ShowNotifications.ErrorNotification)
        End If
    End Sub
    Private Sub GVPNCAcciones_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GVPNCAcciones.SelectedIndexChanged
        If CInt(Session("IdPNC")) > 0 And CInt(Session("IdCausa")) > 0 Then
            Session("IdAccion") = CInt(GVPNCAcciones.SelectedRow().Cells(3).Text)

            TipoAccion.SelectedValue = o.ObtenerTipoAccion(CInt(GVPNCAcciones.SelectedRow().Cells(3).Text))
            txtAccion.Text = CType(GVPNCAcciones.SelectedRow().Cells(5).FindControl("DescripcionAccion"), TextBox).Text
            FechaPlanAccion.Text = GVPNCAcciones.SelectedRow().Cells(6).Text
            FechaEjecAccion.Text = GVPNCAcciones.SelectedRow().Cells(7).Text
            ResponsableAccion.SelectedValue = o.ObtenerResponsableAccion(CInt(GVPNCAcciones.SelectedRow().Cells(3).Text))
            ResponsableSeguir.SelectedValue = o.ObtenerResponsableSeguir(CInt(GVPNCAcciones.SelectedRow().Cells(3).Text))
            txtEvidencia.Text = CType(GVPNCAcciones.SelectedRow().Cells(10).FindControl("EvidenciaCierre"), TextBox).Text
        ...
        Else
            GVPNCAcciones.SelectedRowStyle.ForeColor = Drawing.Color.Black
            GVPNCAcciones.SelectedRowStyle.Font.Bold = False
            ShowNotification("DEBE SELECCIONAR UN PNC y UNA CAUSA ANTES DE SELECCIONAR UNA ACCION", WebMatrix.ShowNotifications.ErrorNotification)
        End If
    End Sub
        ...
        If NEstudio.Items.Count > 1 And NEstudio.SelectedValue <> "-1" Then
            ...
            If rdTrabajo.Checked = True Then
                ...
                Else
                    ...
                    NomTrabajo.Text = NEstudio.SelectedItem.Text
                    Tarea.SelectedValue = "-1"
                End If
            Else
                JobBook.Text = Session("JobBookExterno")
    ...
    End Sub
    Private Sub LimpiarCausaAccion()
        txtCausa.Text = Nothing
        TipoAccion.SelectedValue = "-1"
        txtAccion.Text = Nothing
        ResponsableAccion.SelectedValue = "-1"
