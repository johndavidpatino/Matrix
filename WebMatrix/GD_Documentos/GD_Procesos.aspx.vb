...
Public Class GD_Procesos
    ...
    Public Function ConsultarProceso(ByVal Nombre As String) As List(Of GD_Procesos_Get_F_Result)
        Dim Data As New GD.GD_Procedimientos
        Dim Info As List(Of GD_Procesos_Get_F_Result)
        Try
            Info = Data.ObtenerProcesoxNombre(txtBuscar.Text)
            Return Info
    ...
    Sub CargarGrid(ByVal Opcion As Integer)
        Select Case Opcion
            Case 1
                Session("ListaCargada") = ConsultarProceso(txtBuscar.Text)
    ...
    Protected Sub gvDatos_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvDatos.RowCommand
        Try
            Select Case e.CommandName
                Case "Editar"
                    lblResult.Text = String.Empty
                    Dim Id As Int32 = Int32.Parse(gvDatos.DataKeys(CInt(e.CommandArgument))("IdProceso"))
                    txtId.Text = Id
                    txtId.Enabled = False
                    txtNombre.Text = gvDatos.DataKeys(CInt(e.CommandArgument))("Proceso")
                    datos.Visible = True
    ...
    Protected Sub btnGuardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGuardar.Click
        If Validar() = True Then
            Dim GD As New GD.GD_Procedimientos
            GD.GuardarProceso(txtNombre.Text)
            lblResult.Text = "Proceso agregado correctamente"
            Limpiar()
            CargarGrid(1)
            lista.Visible = True
            datos.Visible = False
    ...
    Protected Sub btnEditar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEditar.Click
        If Validar() = True Then
            Dim GD As New GD.GD_Procedimientos
            GD.EditarProceso(txtId.Text, txtNombre.Text)
            lblResult.Text = "Proceso editado correctamente"
            Limpiar()
            CargarGrid(1)
            lista.Visible = True
            datos.Visible = False
