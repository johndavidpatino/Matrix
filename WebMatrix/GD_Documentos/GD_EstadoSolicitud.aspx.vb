...
Public Class EstadoSolicitud
    ...
    Public Function ConsultarEstadoSolicitud(ByVal Nombre As String) As List(Of GD_EstadoSolicitud_Get_F_Result)
        Dim Data As New GD.GD_Procedimientos
        Dim Info As List(Of GD_EstadoSolicitud_Get_F_Result)
        Try
            Info = Data.ObtenerEstadoSolicitudxNombre(txtBuscar.Text)
            Return Info
    ...
    Sub CargarGrid(ByVal Opcion As Integer)
        Select Case Opcion
            Case 1
                Session("ListaCargada") = ConsultarEstadoSolicitud(txtBuscar.Text)
        ...
        Try
            Select Case e.CommandName
                Case "Editar"
                    ...
                    txtId.Text = Id
                    txtId.Enabled = False
                    txtNombre.Text = gvDatos.DataKeys(CInt(e.CommandArgument))("Estado")
                    datos.Visible = True
    ...
    Protected Sub btnGuardar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGuardar.Click
        If Validar() = True Then
            Dim GD As New GD.GD_Procedimientos
            GD.GuardarEstadoSolicitud(txtNombre.Text)
            lblResult.Text = "Estado Solicitud agregado correctamente"
            Limpiar()
            CargarGrid(1)
            lista.Visible = True
            datos.Visible = False
    ...
    Protected Sub btnEditar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnEditar.Click
        If Validar() = True Then
            Dim GD As New GD.GD_Procedimientos
            GD.EditarEstadoSolicitud(txtId.Text, txtNombre.Text)
            lblResult.Text = "Estado Solicitud editado correctamente"
            Limpiar()
            CargarGrid(1)
            lista.Visible = True
            datos.Visible = False
