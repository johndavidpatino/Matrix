...
#Region "Eventos del Control"
    ...
    Protected Sub btnRechazar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRechazar.Click
        ...
        Dim Registro As New RevisionInstructivo
        Dim oItemsRevision As New List(Of ItemsRevision)
        If ViewState("ItemsRevision") IsNot Nothing Then
            oItemsRevision = CType(ViewState("ItemsRevision"), List(Of ItemsRevision))
        End If
        ...
        Dim RegistroEje As New EjecucionInstructivo
        Dim oItemsEjecucion As New List(Of ItemsEjecucion)
        If ViewState("ItemsEjecucion") IsNot Nothing Then
            oItemsEjecucion = CType(ViewState("ItemsEjecucion"), List(Of ItemsEjecucion))
        End If
...
#Region "Funciones y Metodos"
    ...
    Public Sub CargarInfo()
        Try
            ...
            ''Revisión
            Dim oRevision As New RevisionInstructivo
            Dim olistarevision = oRevision.DevolverxTrabajoID(TrabajoID)
            Dim oItemsrevision As New List(Of ItemsRevision)
            For Each fila As OP_RO_RevisionInstructivo_Get_Result In olistarevision
            ...
            ''Ejecución
            Dim oEjecucion As New EjecucionInstructivo
            Dim olistaejecucion = oEjecucion.DevolverxTrabajoID(TrabajoID)
            Dim oItemsEjecucion As New List(Of ItemsEjecucion)
            For Each fila As OP_RO_EjecucionInstructivo_Get_Result In olistaejecucion
    ...
    Sub EnviarEmail()
        ...
        Try
            ...
            End If
            If Request.QueryString("fromgerencia") IsNot Nothing Then
                oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/ObservacionesRO.aspx?idTrabajo=" & hfidtrabajo.Value & "&tipoRO=Instructivo&fromgerencia=yes")
            Else
                oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/ObservacionesRO.aspx?idTrabajo=" & hfidtrabajo.Value & "&tipoRO=Instructivo")
            End If
