...
#Region "Eventos del Control"
        ...
        If Not IsPostBack Then
            If Request.QueryString("idtrabajo") IsNot Nothing Then
                ...
                Else
                    ...
                    Me.gvRevision.Columns(7).Visible = False
                    Me.gvEjecucion.Columns(7).Visible = False
    ...
    Protected Sub gvRevision_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gvRevision.RowUpdating
        Try
            ...
            Dim otxtPregunta As TextBox = CType(fila.FindControl("txtPreguntaedit"), TextBox)
            ...
            If String.IsNullOrEmpty(otxtPregunta.Text) Then
                Throw New Exception("Debe agregar por lo menos una pregunta")
            End If


            oItemsRevision(Index).Pregunta = otxtPregunta.Text
    ...
    Protected Sub gvEjecucion_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gvEjecucion.RowUpdating
        Try
            ...
            oItemsEjecucion(Index).Pregunta = otxtPregunta.Text
    ...
    Protected Sub btnRechazar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnRechazar.Click
        ...
        Dim Registro As New RevisionCuestionario
        Dim oItemsRevision As New List(Of ItemsRevision)
        If ViewState("ItemsRevision") IsNot Nothing Then
            oItemsRevision = CType(ViewState("ItemsRevision"), List(Of ItemsRevision))
        End If
        For Each row As GridViewRow In gvRevision.Rows
            ...
            If blnCheckedRev = True Then
                Registro.Guardar(oItemsRevision(index).ID, oItemsRevision(index).TrabajoId, oItemsRevision(index).Pregunta, "R", oItemsRevision(index).DescripcionObservacion, oItemsRevision(index).RespuestaGerente)
            End If
            index = index + 1
        Next
        ...
        Dim Registroeje As New EjecucionCuestionario
        Dim oItemsEjecucion As New List(Of ItemsEjecucion)
        If ViewState("ItemsEjecucion") IsNot Nothing Then
            oItemsEjecucion = CType(ViewState("ItemsEjecucion"), List(Of ItemsEjecucion))
        End If
        For Each row2 As GridViewRow In gvEjecucion.Rows
            Dim blnCheckedEje As Boolean = (DirectCast(row2.FindControl("chbRechazarEje"), CheckBox)).Checked
            If blnCheckedEje = True Then
                Registroeje.Guardar(oItemsEjecucion(indexeje).ID, oItemsEjecucion(indexeje).TrabajoId, oItemsEjecucion(indexeje).Pregunta, "R", oItemsEjecucion(indexeje).DescripcionObservacion, oItemsEjecucion(indexeje).RespuestaGerente)
            End If
            indexeje = indexeje + 1
        Next
...
#Region "Funciones y Metodos"
    ...
    Public Sub CargarInfo()
        Try
            ...
            ''Revisión
            Dim oRevision As New RevisionCuestionario
            Dim olistarevision = oRevision.DevolverxTrabajoID(TrabajoID)
            Dim oItemsrevision As New List(Of ItemsRevision)
            For Each fila As OP_RO_RevisionCuestionario_Get_Result In olistarevision
                oItemsrevision.Add(New ItemsRevision With {.ID = fila.id, .DescripcionObservacion = fila.DescripcionObservacion, .FechaHoraObservacion = fila.FechaHoraObservacion, .FechaHoraRespuesta = fila.FechaHoraRespuesta,
                                                             .Observacion = fila.Observacion, .Pregunta = fila.Pregunta, .RespuestaGerente = fila.RespuestaGerenteProyectos, .TrabajoId = fila.TrabajoId})
            Next
            ...
            ''Ejecución
            Dim oEjecucion As New EjecucionCuestionario
            Dim olistaejecucion = oEjecucion.DevolverxTrabajoID(TrabajoID)
            Dim oItemsEjecucion As New List(Of ItemsEjecucion)
            For Each fila As OP_RO_EjecucionCuestionario_Get_Result In olistaejecucion
    ...
    Public Sub AgregarRevision(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            If gvRevision.Rows.Count > 0 Then
                ...
                Dim otxtPreguntafooter As TextBox = CType(ofooter.FindControl("txtPreguntafooter"), TextBox)
                ...
                If String.IsNullOrEmpty(otxtPreguntafooter.Text) Then
                    Throw New Exception("Debe agregar por lo menos una pregunta")
                End If

                oItemsRevision.Add(New ItemsRevision With {.ID = -1, .Pregunta = otxtPreguntafooter.Text, .Observacion = otxtObservacionfooter.Text,
            ...
            Else
                ...
                Dim otxtPreguntaEmpty As TextBox = CType(oControl.FindControl("txtPreguntaEmpty"), TextBox)
                ...
                If String.IsNullOrEmpty(otxtPreguntaEmpty.Text) Then
                    Throw New Exception("Debe agregar por lo menos una pregunta")
                End If

                oItemsRevision.Add(New ItemsRevision With {.ID = -1, .Pregunta = otxtPreguntaEmpty.Text, .Observacion = otxtObservacionEmpty.Text,
    ...
    Public Sub AgregarEjecucion(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            If gvEjecucion.Rows.Count > 0 Then
                ...
                oItemsEjecucion.Add(New ItemsEjecucion With {.ID = -1, .Pregunta = otxtPreguntafooter.Text, .Observacion = otxtObservacionfooter.Text,
                                                           .DescripcionObservacion = otxtDescripcionfooter.Text, .RespuestaGerente = otxtEspaciofooter.Text, .TrabajoId = TrabajoID})
            ...
            Else
                ...
                oItemsEjecucion.Add(New ItemsEjecucion With {.ID = -1, .Pregunta = otxtPreguntaEmpty.Text, .Observacion = otxtObservacionEmpty.Text,
                                                           .DescripcionObservacion = otxtDescripcionEmpty.Text, .RespuestaGerente = otxtEspacioEmpty.Text, .TrabajoId = TrabajoID})
    ...
    Public Sub GuardarCuestionario()
        Try
            ...
            For Each fila As ItemsRevision In oItemsRevision
                oRevision.Guardar(fila.ID, TrabajoID, fila.Pregunta, fila.Observacion, fila.DescripcionObservacion, fila.RespuestaGerente)
            Next
            ...
            For Each fila As ItemsEjecucion In oItemsEjecucion
                oEjecucion.Guardar(fila.ID, TrabajoID, fila.Pregunta, fila.Observacion, fila.DescripcionObservacion, fila.RespuestaGerente)
            Next
    ...
    Sub EnviarEmail()
        ...
        Try
            ...
            If Request.QueryString("fromgerencia") IsNot Nothing Then
                oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/ObservacionesRO.aspx?idTrabajo=" & hfidtrabajo.Value & "&tipoRO=Cuestionario&fromgerencia=yes")
            Else
                oEnviarCorreo.enviarCorreo(WebMatrix.Util.obtenerUrlRaiz() & "/Emails/ObservacionesRO.aspx?idTrabajo=" & hfidtrabajo.Value & "&tipoRO=Cuestionario")
            End If
