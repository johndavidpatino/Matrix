...
#Region "Enumeradores"
    Enum eTarea
        ...
        ControlInternoScripting = 76
		ControlInternoProcesamiento = 78
		ControlInternoCodificacion = 81
	End Enum
...
#Region "Eventos del Control"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Request.QueryString("idtrabajo") IsNot Nothing Then
            ...
            lblNombreTarea.Text = tareaactual.Tarea

            If tareaactual.TareaId = eTarea.Instrumentos Then
                Me.gvRevision.Columns(3).Visible = True
            Else
                Me.gvRevision.Columns(3).Visible = False
            End If

            If tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting Then
                Me.gvRevision.Columns(4).Visible = True
            Else
                Me.gvRevision.Columns(4).Visible = False
            End If

            If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Then
                Me.gvRevision.Columns(5).Visible = True
            Else
                Me.gvRevision.Columns(5).Visible = False
            End If

			If tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion Or tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaCOE OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaGP Then
				Me.gvRevision.Columns(6).Visible = True
			Else
    ...
    Private Sub gvRevision_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvRevision.RowDataBound
        ...
        If Request.QueryString("fromgerencia") IsNot Nothing Then
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If (e.Row.RowState And DataControlRowState.Edit) > 0 Then
					...
					If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaCOE OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaGP Then

						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).DataSource = o.ProcesosListXTarea(tareaactual.TareaId)
						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).DataTextField = "Proceso"
						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).DataValueField = "Id"
						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).DataBind()
						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).Items.Insert(0, New ListItem With {.Text = "--Seleccione--", .Value = "0"})
						DirectCast(e.Row.FindControl("ddlProcesoedit"), DropDownList).SelectedValue = CType(e.Row.DataItem, OP_IPS_Revision_Get_Result).CodProceso
        ...
        If e.Row.RowType = DataControlRowType.EmptyDataRow Then
            ...
            If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Then
                DirectCast(e.Row.FindControl("ddlAplicativoEmpty"), DropDownList).Visible = True
                e.Row.FindControl("colAplicativo").Visible = True
                e.Row.FindControl("colAplicativo2").Visible = True
            Else
			...
			If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaCOE OrElse tareaactual.TareaId = eTarea.VariablesControlEvaluaGP Then
				DirectCast(e.Row.FindControl("ddlProcesoEmpty"), DropDownList).Visible = True
				e.Row.FindControl("colProceso").Visible = True
				e.Row.FindControl("colProceso2").Visible = True
			Else
            ...
            If (tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting) Then
                DirectCast(e.Row.FindControl("txtPreguntaEmpty"), TextBox).Visible = True
                e.Row.FindControl("colPregunta").Visible = True
                e.Row.FindControl("colPregunta2").Visible = True
            Else
            ...
            DirectCast(e.Row.FindControl("ddlProcesoEmpty"), DropDownList).DataSource = o.ProcesosListXTarea(tareaactual.TareaId)
            DirectCast(e.Row.FindControl("ddlProcesoEmpty"), DropDownList).DataTextField = "Proceso"
            DirectCast(e.Row.FindControl("ddlProcesoEmpty"), DropDownList).DataValueField = "Id"
            DirectCast(e.Row.FindControl("ddlProcesoEmpty"), DropDownList).DataBind()
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            DirectCast(e.Row.FindControl("ddlProcesofooter"), DropDownList).DataSource = o.ProcesosListXTarea(tareaactual.TareaId)
            DirectCast(e.Row.FindControl("ddlProcesofooter"), DropDownList).DataTextField = "Proceso"
            DirectCast(e.Row.FindControl("ddlProcesofooter"), DropDownList).DataValueField = "Id"
            DirectCast(e.Row.FindControl("ddlProcesofooter"), DropDownList).DataBind()
    ...
    Protected Sub gvRevision_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gvRevision.RowUpdating
        ...
        If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Then
            odllAplicativo = CType(fila.FindControl("ddlAplicativoedit"), DropDownList).SelectedValue
        End If

		If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion Then
			odllProceso = CType(fila.FindControl("ddlProcesoedit"), DropDownList).SelectedValue
		End If

		otxtVersion = CType(fila.FindControl("txtVersionedit"), TextBox).Text

        If tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting Then
            otxtPregunta = CType(fila.FindControl("txtPreguntaedit"), TextBox).Text
        Else
        ...
        If (tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting) And String.IsNullOrEmpty(otxtPregunta) Then
            Throw New Exception("Debe agregar por lo menos una pregunta")
        End If
...
#Region "Funciones y Metodos"
    ...
    Public Sub AgregarRevision(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If gvRevision.Rows.Count > 0 Then
            ...
            If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Then
                odllAplicativofooter = CType(ofooter.FindControl("ddlAplicativofooter"), DropDownList).SelectedValue
            End If

			If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion Then
				odllProcesofooter = CType(ofooter.FindControl("ddlProcesofooter"), DropDownList).SelectedValue
			End If

			otxtVersionfooter = CType(ofooter.FindControl("txtVersionfooter"), TextBox).Text

            If tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting Then
                otxtPreguntafooter = CType(ofooter.FindControl("txtPreguntafooter"), TextBox).Text
            Else
                otxtPreguntafooter = Nothing
            End If

            If tareaactual.TareaId = eTarea.Instrumentos And otxtInstrumentofooter = 0 Then
            ...
            If (tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting) And odllAplicativofooter = "-1" Then
                ShowNotification("Debe seleccionar el Aplicativo", ShowNotifications.ErrorNotification)
                ofooter.FindControl("ddlAplicativofooter").Focus()
                Exit Sub
            End If

			If (tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion) And odllProcesofooter = "-1" Then
				ShowNotification("Debe seleccionar el Proceso", ShowNotifications.ErrorNotification)
				ofooter.FindControl("ddlProcesofooter").Focus()
				Exit Sub
			End If
            ...
            If (tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting) And String.IsNullOrEmpty(otxtPreguntafooter) Then
                ShowNotification("Debe agregar por lo menos una pregunta", ShowNotifications.ErrorNotification)
                ofooter.FindControl("txtPreguntafooter").Focus()
                Exit Sub
            End If
        ...
        Else
            ...
            If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Then
                odllAplicativoEmpty = CType(oControl.FindControl("ddlAplicativoEmpty"), DropDownList).SelectedValue
            End If

			If tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion Then
				odllProcesoEmpty = CType(oControl.FindControl("ddlProcesoEmpty"), DropDownList).SelectedValue
			End If
            ...
            If tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting Then
                otxtPreguntaEmpty = CType(oControl.FindControl("txtPreguntaEmpty"), TextBox).Text
            Else
                otxtPreguntaEmpty = Nothing
            End If

            If tareaactual.TareaId = eTarea.Instrumentos And otxtInstrumentoEmpty = 0 Then
                ShowNotification("Debe seleccionar el tipo de instrumento", ShowNotifications.ErrorNotification)
                oControl.FindControl("txtInstrumentoEmpty").Focus()
                Exit Sub
            End If

            If (tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento) And odllAplicativoEmpty = "-1" Then
                ShowNotification("Debe seleccionar el Aplicativo", ShowNotifications.ErrorNotification)
                oControl.FindControl("ddlAplicativoEmpty").Focus()
                Exit Sub
            End If

			If (tareaactual.TareaId = eTarea.Procesamiento Or tareaactual.TareaId = eTarea.ControlInternoProcesamiento Or tareaactual.TareaId = eTarea.Codificacion Or tareaactual.TareaId = eTarea.ControlInternoCodificacion) And odllProcesoEmpty = "-1" Then
				ShowNotification("Debe seleccionar el Proceso", ShowNotifications.ErrorNotification)
				oControl.FindControl("ddlProcesoEmpty").Focus()
				Exit Sub
			End If
            ...
            If (tareaactual.TareaId = eTarea.Instrumentos Or tareaactual.TareaId = eTarea.Scripting Or tareaactual.TareaId = eTarea.ControlInternoScripting) And String.IsNullOrEmpty(otxtPreguntaEmpty) Then
                ShowNotification("Debe agregar por lo menos una pregunta", ShowNotifications.ErrorNotification)
                oControl.FindControl("txtPreguntaEmpty").Focus()
                Exit Sub
            End If
