...
Public Class RevisionProductividadCoordinador
    ...
    Sub CargarTrabajos()
        ...
        If Now.Month = 1 Then inicioCorteFecha = DateAdd(DateInterval.Year, -1, inicioCorteFecha)
        Dim finCorteFecha = New DateTime(Now.Year, Now.Month, 15)
        ddlTrabajoSeleccionado.DataSource = op.CuantiProdProductividadTrabajos_Get(inicioCorteFecha, finCorteFecha, Session("IDUsuario").ToString, Nothing, Nothing, Nothing, False, Nothing, Nothing, Nothing)
    ...
    Sub CargarStatus()
        ...
        Dim finCorteFecha = New DateTime(Now.Year, Now.Month, 15)
        Dim op As New OP_CuantiDapper
        gvEstatus.DataSource = op.CuantiProdProductividadStatus_Get(inicioCorteFecha, finCorteFecha, Session("IDUsuario").ToString, Nothing, Nothing, Nothing, False, Nothing, Nothing)
        gvEstatus.DataBind()
    End Sub
    ...
    Sub CargarGrid()
        ...
        Dim op As New OP_CuantiDapper
        gvDataSearch.DataSource = op.CuantiProdProductividad_Get(inicioCorteFecha, finCorteFecha, Session("IDUsuario").ToString, Nothing, ddlTrabajoSeleccionado.SelectedValue, Nothing, False, Nothing, Nothing, Nothing)
    ...
    Protected Sub btnGuardar_Click(sender As Object, e As EventArgs)
        ...
        For Each row As GridViewRow In gvDataSearch.Rows
            If row.RowType = DataControlRowType.DataRow Then
                ...
                ent.ObservacionesCoordinador = DirectCast(Me.gvDataSearch.Rows(row.RowIndex).FindControl("txtObservaciones"), TextBox).Text
            End If
        Next
