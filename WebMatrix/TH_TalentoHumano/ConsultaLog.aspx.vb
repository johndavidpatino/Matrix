...
#Region "Metodos"
    ...
    Public Sub ObtenerInformacion(ByVal Fecini As Date?, ByVal Fecfin As Date?, ByVal Cedula As Int64?)
        ...
        gvPersonas.DataSource = op.Th_GetLogPersonas(Fecini, Fecfin, Cedula, If(ddlTipoCambio.SelectedValue = "", CType(Nothing, String), ddlTipoCambio.SelectedValue))
        gvPersonas.DataBind()
        ActivateAccordion(0, EffectActivateAccordion.SlideEffect)
    End Sub
    Protected Sub Exportar(ByVal Fecini As Date?, ByVal Fecfin As Date?, ByVal Cedula As Int64?)
        Dim vTitulos As String() = "PersonaId;Nombre;Cambio;ValorOriginal;ValorNuevo;Fecha;UsuarioId;NombreUsuario;TextoValorOriginal;TextoValorNuevo".Split(";")
        Dim lstCambios As List(Of TH_GetLogPersonas_Result)
        Dim workbook = New XLWorkbook()
        Dim worksheet = workbook.Worksheets.Add("ListadoCambiosPersonas")


        Dim op As New Personas
        lstCambios = op.Th_GetLogPersonas(Fecini, Fecfin, Cedula, If(ddlTipoCambio.SelectedValue = "", CType(Nothing, String), ddlTipoCambio.SelectedValue))
        ...
        Crearexcel(workbook, "ListadoCambiosPersonas -" & Now.Year & "-" & Now.Month & "-" & Now.Day)
    End Sub
