...
Public Class TH_SolicitudAusencia
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            If Request.QueryString("Request") IsNot Nothing Then
                Select Case Request.QueryString("Request")
                    Case "New"
                ...
                End Select
            End If
            Dim ausencisaEquipo As New AusenciasEquipoDapper()
            CargarTiposSolicitudesAusencia()
            CargarAprobador()
            Dim permisos As New Datos.ClsPermisosUsuarios
            Dim UsuarioID As Int64 = Int64.Parse(Session("IDUsuario").ToString())

            Dim aprobaciones = ausencisaEquipo.GetAusenciasPersonas(Session("IDUsuario"), "")

            'Se habilita el menu de ausencias equipo si tiene aprobaciones
            If permisos.VerificarRolUsuario(55, UsuarioID) = True Then
                pnlRHIncapacidadEmpleado.Visible = True
                CargarEmpleados()
            Else
                pnlRHIncapacidadEmpleado.Visible = False
            End If
        End If
    ...
    'Private Sub lbMenuAusenciasEquipo_Click(sender As Object, e As EventArgs) Handles lbMenuAusenciasEquipo.Click
    '    Response.Redirect("../TH_TalentoHumano/AusenciasEquipo.aspx")
    'End Sub
    ...
    Private Sub btnIncapacidadSubmit_Click(sender As Object, e As EventArgs) Handles btnIncapacidadSubmit.Click
        ...
        Dim o As New TH_Ausencia.DAL
        'Dim validacion = o.ValidarSolicitudAusencia(Session("IDUsuario").ToString, txtFechaInicioSolicitud.Text, txtFechaFinSolicitud.Text, ddlTipoSolicitud.SelectedValue)(0)
        Dim idusuario As Int64 = 0
        If pnlRHIncapacidadEmpleado.Visible = True Then
            idusuario = ddlEmpleadoIncapacidad.SelectedValue
        Else
            idusuario = Session("IDUsuario").ToString
        End If
        Dim validacion = o.ValidarSolicitudAusencia(idusuario, txtFechaInicioSolicitud.Text, txtFechaFinSolicitud.Text, ddlTipoSolicitud.SelectedValue)(0)
        If validacion.Result = 0 Then
            AlertJS(validacion.MessageResult)
            Exit Sub
        End If
    ...
    Sub CargarEmpleados()
        Dim o As New TH_Ausencia.DAL
        ddlEmpleadoIncapacidad.DataSource = o.ObtenerAprobadores
        ddlEmpleadoIncapacidad.DataTextField = "NombreCompleto"
        ddlEmpleadoIncapacidad.DataValueField = "id"
        ddlEmpleadoIncapacidad.DataBind()
        ddlEmpleadoIncapacidad.SelectedValue = Session("IDUsuario").ToString
    End Sub
    ...
    Sub EnvioCorreoDefinicionAusencia(ByVal idSolicitud As Int64)
        Dim oEnviarCorreo As New EnviarCorreo
    ...
    Private Sub gvAprobacionesPendientes_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAprobacionesPendientes.RowCommand
        ...
        If e.CommandName = "Aprobar" Then
            ...
            ElseIf ent.Tipo = 2 Then
                ent.Estado = 20
                ent.VoBo1 = Session("IDUsuario").ToString
                ent.FechaVoBo1 = Date.UtcNow.AddHours(-5)
                o.SaveChangesSolicitud(ent)
                EnvioCorreoDefinicionAusencia(ent.id)
            ElseIf ent.Tipo = 4 Then
                ent.Estado = 20
                ent.VoBo1 = Session("IDUsuario").ToString
                ent.FechaVoBo1 = Date.UtcNow.AddHours(-5)
                o.SaveChangesSolicitud(ent)
