...
<Serializable()>
Public Class FichaObservaciones
...
#End Region
#Region "Obtener"
    Public Function DevolverTodos() As List(Of OP_FichaObservaciones_Get_Result)
        Try
            Return OP_FichaObservaciones_Get(Nothing, Nothing)
        ...
        End Try
    End Function
    Public Function DevolverxTrabajoID(ByVal TrabajoID As Int64?) As List(Of OP_FichaObservaciones_Get_Result)
        Try
            Return OP_FichaObservaciones_Get(Nothing, TrabajoID)
        Catch ex As Exception
            If (ex.InnerException Is Nothing) Then
                Throw ex
            Else
        ...
        End Try
    End Function
    Public Function DevolverxID(ByVal ID As Int64) As OP_FichaObservaciones_Get_Result
        Try
            Dim oResult As List(Of OP_FichaObservaciones_Get_Result)
            oResult = OP_FichaObservaciones_Get(ID, Nothing)
        ...
        End Try
    End Function
    Private Function OP_FichaObservaciones_Get(ByVal ID As Int64, ByVal TrabajoID As Int64) As List(Of OP_FichaObservaciones_Get_Result)
        Try
            Dim oResult As ObjectResult(Of OP_FichaObservaciones_Get_Result) = oMatrixContext.OP_FichaObservaciones_Get(ID, TrabajoID)
...
#Region "Guardar"
    Public Function Guardar(ByVal ID As Int64?, ByVal TrabajoId As Int64?, ByVal CantidadRequerida As Short?, ByVal FlashReport As Boolean?, ByVal DescripcionIncentivos As String,
        ...
        Try
            Dim FichaObservacionesID As Decimal = 0

            If ID > 0 Then
                oMatrixContext.OP_FichaObservaciones_Edit(ID, TrabajoId, CantidadRequerida, FlashReport, DescripcionIncentivos, IncentivoEconomico, PresupuestoIncentivo, RegalosCliente, CompraIpsos, Presupuesto, CircuitoCerrado, FilmacionFija, CamaraFotografica,
                ...
                FichaObservacionesID = ID
            Else
                Dim oResult As ObjectResult(Of Decimal?)
                oResult = oMatrixContext.OP_FichaObservaciones_Add(TrabajoId, CantidadRequerida, FlashReport, DescripcionIncentivos, IncentivoEconomico, PresupuestoIncentivo, RegalosCliente, CompraIpsos, Presupuesto, CircuitoCerrado, FilmacionFija, CamaraFotografica,
                             Tv_DVD, FilmacionActiva, VideoBeam, EntregaFiltrosReclutamiento, EntregaFiltrosAsistente, EntregaCartaInvitacion, EntregaFaxConfirmacion, ListadosCliente, CallCenter, SacaCita, FlashReportEscrito, FlashReportVerbal, Transcripcion, Grabacion,
                             GrupoObjetivo, CaracteristicasEspeciales, Comentarios, MetodoAceptableReclutamiento, ExclusionesYRestriccionesEspecificas, RecursosPropiedadCliente, Observaciones)
                FichaObservacionesID = Decimal.Parse(oResult.ToList().Item(0))
            End If

            Return FichaObservacionesID
        Catch ex As Exception
            If (ex.InnerException Is Nothing) Then
                Throw ex
            Else
...
#Region "Eliminar"
    Public Function Eliminar(ByVal ID As Int64) As Integer
        Try
            Return oMatrixContext.OP_FichaObservaciones_Del(ID)
        Catch ex As Exception
            If (ex.InnerException Is Nothing) Then
                Throw ex
            Else
