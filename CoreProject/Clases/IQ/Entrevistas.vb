    ...
    Public Class Entrevistas
        ...
        Public Function ObtenerSesiones(ByVal par As IQ_Parametros) As IQ_Parametros
            Try
                ...
                Dim P1 As IQ_Parametros
                P1 = (From p In _IQ_Entities.IQ_Parametros Where p.IdPropuesta = par.IdPropuesta And p.ParAlternativa = par.ParAlternativa And p.TecCodigo = par.TecCodigo And p.ParNacional = par.ParNacional Select p).First()
        ...
        Public Sub InsertarPresupuesto(ByVal par As IQ_Parametros, ByVal lstMuestra As List(Of IQ_Muestra_1), ByVal lstProc As List(Of IQ_ProcesosPresupuesto), ByVal lstOp As List(Of IQ_OpcionesAplicadas))
            Try
        ...
        Public Function ExisteOpcion(ByVal lst As List(Of IQ_OpcionesAplicadas), ByVal op As Integer)
            Try
                ...
                If lst.Any(Function(p) p.IdOpcion = op) Then
                    existe = True
                End If
        ...
        Public Function ObtenerCostoDirecto(ByVal par As IQ_Parametros) As Decimal
            ...
            Try
                _IQ_Entities = New IQ_MODEL
                Dim output As New Objects.ObjectParameter("ENTREVISTASCostoDirecto", GetType(Decimal))

                _IQ_Entities.IQ_ENTREVISTASCostosDirectos(par.IdPropuesta, par.ParAlternativa, par.MetCodigo, par.ParNacional, output)
